...
//    likeBtn.backgroundColor = [UIColor whiteColor];
    [likeBtn setImage:[UIImage ml_imageFromBundleNamed:@"empty_like.png"] forState:(UIControlStateNormal)];
...
- (void)likeBtnAction:(UIButton *)sender{
    ZLPhotoPickerBrowserPhoto *photo = [self.photos objectAtIndex:self.currentPage];
    BOOL isClick = [MyDefaults boolForKey:[NSString stringWithFormat:@"%@",photo.b34]];
    if (isClick) {
        [self disLike];
    }else{
      [self like];
...
- (void)like{
    ...
    ZLPhotoPickerBrowserPhoto *photo = [self.photos objectAtIndex:self.currentPage];
    NSString *a109 = userId;
    NSString *a108 = photo.b34;
    NSString *a52 = userinfo.b52;
    ...
    NSString *url = [NSString stringWithFormat:@"%@f_113_10_5.service?a109=%@&a108=%@&a52=%@&p1=%@&p2=%@&%@",kServerAddressTest2, a109, a108,a52,sessonId,userId,appinfoStr];
    ...
    [manger GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([codeNum integerValue] == 200) {
            // 保存点赞的记录id，为了取消点赞
            [[NSUserDefaults standardUserDefaults] setObject:[[infoDic objectForKey:@"body"] objectForKey:@"b34"] forKey:[NSString stringWithFormat:@"%@:%@",a109,a108]];
            [MyDefaults setBool:YES forKey:[NSString stringWithFormat:@"%@",photo.b34]];
...
- (void)disLike{
    NSString *userId = [NSString stringWithFormat:@"%@",[NSGetTools getUserID]];
//    DHUserInfoModel *userinfo = [DBManager getUserWithCurrentUserId:userId];
    ZLPhotoPickerBrowserPhoto *photo = [self.photos objectAtIndex:self.currentPage];
    NSString *a109 = userId;
    NSString *a108 = photo.b34;
    NSString *sessonId = [NSGetTools getUserSessionId];
    ...
    manger.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"application/json"];
    NSString *appinfoStr = [NSGetTools getAppInfoString];
    NSString *url = [NSString stringWithFormat:@"%@f_113_11_5.service?a109=%@&a108=%@&p1=%@&p2=%@&%@",kServerAddressTest2, a109, a108,sessonId,userId,appinfoStr];
    url = [url stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding];
    [manger GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *datas = responseObject;
        NSString *result = [[NSString alloc] initWithData:datas encoding:NSUTF8StringEncoding];
        ...
        if ([codeNum integerValue] == 200) {
            // 保存点赞的记录id，为了取消点赞
            [MyDefaults setBool:NO forKey:[NSString stringWithFormat:@"%@",photo.b34]];
            dispatch_async(dispatch_get_main_queue(), ^{
                [self showHint:@"取消点赞"];
            });
        }
