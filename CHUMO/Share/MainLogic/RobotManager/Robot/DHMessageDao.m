...
- (void)createTable{
    NSString *createTableSQL = @"CREATE TABLE MESSAGETABLE (messageId text,messageType text,message text,timeStamp text,fromUserAccount text,fromUserDevice text,toUserAccount text,token text,roomCode text,roomName text,userId text,targetId text,robotMessageType text,isRead text,targetUserType text,fileUrl text,fileDerual text)";
...
- (void)insertMessageDataDBWithModel:(DHMessageModel *)item userId:(NSString *)userId{
    NSString * insertSql = [NSString stringWithFormat:@"INSERT INTO MESSAGETABLE (messageId ,messageType ,message ,timeStamp ,fromUserAccount ,fromUserDevice ,toUserAccount ,token, roomCode ,roomName ,userId ,targetId,robotMessageType,isRead,targetUserType,fileUrl,fileDerual) VALUES  ('%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%2f')",item.messageId,item.messageType,item.message,item.timeStamp,item.fromUserAccount,item.fromUserDevice,item.toUserAccount,item.token,item.roomCode,item.roomName,userId,item.targetId,item.robotMessageType,item.isRead,item.targetUserType,item.fileUrl,item.fileDuration];
    FMDatabaseQueue *queue = [DBHelper getDatabaseQueue];
    [queue inDatabase:^(FMDatabase *db) {
...
- (NSMutableArray *)getChatListWithUserId:(NSString *)targetId roomCode:(NSString *)roomCode{
    ...
    [queue inDatabase:^(FMDatabase *db) {
        ...
        while ([result next]) {
            ...
            item.fileUrl = [result stringForColumn:@"fileUrl"];
            item.fileDuration = [[result stringForColumn:@"fileDerual"] floatValue];
...
+ (NSMutableArray *)selectMessageDBWithUserId:(NSString *)userId targetId:(NSString *)targetId atIndex:(NSInteger )index{
    return [[DHMessageDao shareInstance] selectMessageDBWithUserId:userId targetId:targetId atIndex:index];
}
- (NSMutableArray *)selectMessageDBWithUserId:(NSString *)userId targetId:(NSString *)targetId atIndex:(NSInteger )index{
    NSMutableArray *allMutableArray = [NSMutableArray array];
//    NSString * sql=[NSString stringWithFormat:@"SELECT * FROM MESSAGETABLE where userId='%@' and targetId='%@' order by timeStamp desc limit %ld,10",userId,targetId,index];
...
+ (NSMutableArray *)selectMessageDBWithUserId:(NSString *)userId targetId:(NSString *)targetId lastTime:(NSString *)lastTime{
    return [[DHMessageDao shareInstance] selectMessageDBWithUserId:userId targetId:targetId lastTime:lastTime];
}
- (NSMutableArray *)selectMessageDBWithUserId:(NSString *)userId targetId:(NSString *)targetId lastTime:(NSString *)lastTime{
    NSMutableArray *allMutableArray = [NSMutableArray array];
    NSString * sql=[NSString stringWithFormat:@"SELECT * FROM MESSAGETABLE where userId='%@' and targetId='%@' and timeStamp< '%@'  order by timeStamp desc limit 0,10",userId,targetId,lastTime];
    //    NSString * sql=[NSString stringWithFormat:@"SELECT * FROM MESSAGETABLE where userId='%@' and targetId='%@' ",userId,targetId];
    FMDatabaseQueue *queue = [DBHelper getDatabaseQueue];
    [queue inDatabase:^(FMDatabase *db) {
        FMResultSet *result = [db executeQuery:sql];
...
+ (DHMessageModel *)getMessageWithMessageId:(NSString *)messageId {
    return [[DHMessageDao shareInstance] getMessageWithMessageId:messageId];
}
- (DHMessageModel *)getMessageWithMessageId:(NSString *)messageId {
    NSMutableArray *allMutableArray = [NSMutableArray array];
    NSString *userId = [NSString stringWithFormat:@"%@",[NSGetTools getUserID]];
    NSString * sql=[NSString stringWithFormat:@"select *from messagetable where userId = '%@' and messageId='%@'",userId,messageId];
    FMDatabaseQueue *queue = [DBHelper getDatabaseQueue];
    [queue inDatabase:^(FMDatabase *db) {
        FMResultSet *result = [db executeQuery:sql];
        while ([result next]) {
    ...
    return [allMutableArray firstObject];
}
